...
正如变量默认不可变一样，**引用指向的值默认也是不可变的**，来看看如何解决这个问题。
...
fn main() {
   let mut s = String::from("hello");
    ...
    let r3 = &mut s;
    println!("{}", r3);
} // 老编译器中，r1、r2、r3作用域在这里结束，（是指到 “let r3 = &mut s;” 结束）
```
...
- **同一时刻，你只能拥有要么一个可变引用, 要么任意多个不可变引用，（同一时刻 可以理解为在你使用的那个时间点）**
- **引用必须总是有效的**
...
fn main() {
    ...
    }
    {
        ...
        println!("b = {}", b); // 这两行不能乱序
        println!("a = {}", a); // 此时 b 已经结束了生命周期 (NLL)
    }
}
```

![](https://tva1.sinaimg.cn/large/006DIypxly1h6raza8mv4j30py0e4adu.jpg)

![](https://tva1.sinaimg.cn/large/006DIypxly1h6razitj3fj30hy0ii778.jpg)
...
- 一些困惑：
  ...
  这段代码的函数`calculate_length`里`s`的类型是`&String`，为什么可以直接就使用`String`类型的`len()`方法，而不是`*s.len()`这样呢？

  而且将这个函数改成下面这样，不管加不加`*`都可以通过编译
...
- 将引用传入函数之后，并没有解引用，是自动解了吗？
  ...
  fn change(some_string: &String) {
      some_string.push_str(", world");    // 这里为啥不是 *some_string.push_str(", world")
  }
  ```
