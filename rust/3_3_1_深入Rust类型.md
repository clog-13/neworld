...
本章将重点学习如何创建自定义类型，以及了解何为动态大小的类型。
...
首先，更好的可读性不等于更少的代码，其次下面的例子只是一个示例，未必能体现出更好的可读性：
...
不过用户依然可以通过 `n.0.pow(2)` 的方式来调用内部类型的方法。
...
在[函数](https://course.rs/basic/base-type/function.html#永不返回的函数)那章，曾经介绍过 `!` 类型：`!` 用来说明一个函数永不返回任何值：
