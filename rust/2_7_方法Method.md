...
在 Rust 中，方法的概念也大差不差，往往和对象class成对出现：
...
![](https://tva1.sinaimg.cn/large/006DIypxly1h6rch3411pj30a90ai3zl.jpg)
...
在 `area` 的签名中，我们使用 `&self` 替代 `rectangle: &Rectangle`，**`&self` 其实是 `self: &Self` 的简写（注意大小写）**。在一个 `impl` 块内，**`Self` 指代被实现方法的结构体类型，`self` 指代此类型的实例**，换句话说，我们为哪个结构体实现方法，那么 `self` 就是指代哪个结构体的实例。
...
为什么最后需要返回&T类型呢？本地尝试了取消引用，提示cannot move out of `self.x` which is behind a shared reference，并且还说T没有copy特征，但是之前不是说了&self相当于self:&Self吗，这么说我直接self.x返回的不就应该是&T类型吗，难道说rust自动解引用了所以需要再引用一次吗？同时，为什么不能返回T类型呢？

代码没有泛型参数的声明。`&self`的确是`self : &Self`，但是`self.x`代表的是`(&Self).x`，意味着通过引用拿出其成员，属于move。所有权规则不允许通过引用拿走所有权，要么你Copy一份自己玩去，原来那一份你动不了。

另外，&self.x的类型是&T，就是那个成员x的引用。先处理self.x，&是最后的。
...
如何为一个结构体定义一个构造器方法？也就是接受几个参数，然后构造并返回该结构体的实例。很简单，**参数中不包含 `self` 即可（第一个参数是`self`或者`&self`的就是方法，否则就是关联函数）**。
