...
来，大哥来告诉你返回什么：`std::result::Result<std::fs::File, std::io::Error>`。

别慌，其实很简单，首先 `Result` 本身是定义在 `std::result` 中的，但是因为 `Result` 很常用，所以就被包含在了 [`prelude`](https://course.rs/appendix/prelude.html) 中（将常用的提前引入到当前作用域内），因此无需手动引入 `std::result::Result`，那么返回类型可以简化为 `Result<std::fs::File,std::io::Error>`，你看看是不是很像标准的 `Result<T, E>` 枚举定义？只不过 `T` 被替换成了具体的类型 `std::fs::File`，是一个文件句柄类型，`E` 被替换成 `std::io::Error`，是一个 IO 错误类型.
...
## [失败就 panic: unwrap 和 expect](https://course.rs/basic/result-error/result.html#失败就-panic-unwrap-和-expect)
...
## [传播错误](https://course.rs/basic/result-error/result.html#传播错误)
...
### [传播界的大明星: ?](https://course.rs/basic/result-error/result.html#传播界的大明星-)
...
#### [? 用于 Option 的返回](https://course.rs/basic/result-error/result.html#-用于-option-的返回)
...
#### [新手用 ? 常会犯的错误](https://course.rs/basic/result-error/result.html#新手用--常会犯的错误)
