...
`Any`类型消息允许你在没有指定`.proto`定义的情况下使用消息作为一个嵌套类型。一个`Any`类型包括一个可以被序列化的`bytes`，以及一个`URL`作为一个全局标识符和解析消息类型。使用`Any`类型，需要导入`google/protobuf/any.proto`。
...
目前，用于`Any`类型的动态库仍在开发之中，如果你熟悉[`proto2`语法](https://developers.google.com/protocol-buffers/docs/proto)，使用`Any`类型替换[扩展](https://developers.google.com/protocol-buffers/docs/proto#extensions)。
...
支持`map`的任何`protocol buffers`实现都必须生成和接受上述定义所表示的数据。
...
- 在`Go`中，除非在`.proto`文件中明确提供选项`go_package`，否则该包将用作`Go`包的名称。
...
- **输出字段的默认值**：在`proto3 JSON`的输出中默认省略字段的默认值。有一个选项可以改为输出字段的默认值。
...
`Protocol Buffers`还允许定义和使用自定义的选项。如果需要创建自定义的选项，请参阅[`proto2`语言指南](https://developers.google.com/protocol-buffers/docs/proto.html#customoptions)以获取详细信息。请注意，创建自定义的选项使用的[扩展](https://developers.google.com/protocol-buffers/docs/proto.html#extensions)仅允许用于`proto3`中的自定义的选项。

## 生成自定义的类

需要使`.proto`文件中定义的消息类型来生成代码，这需要在`.proto`上运行`protocol buffers`编译器`protoc`。对于`Go`，还需要为编译器安装一个特殊的代码生成器插件，可以在`GitHub`上的`golang/protobuf`存储库中找到这个插件和安装说明。
...
- 可以提供一个或多个输出指令：`--go_out`在`DST_DIR`中生成`Go`代码。有关更多信息，请参阅[`Go`生成代码参考](https://developers.google.com/protocol-buffers/docs/reference/go-generated)。

  为方便起见，如果`DST_DIR`以`.zip`或.`jar`结尾，编译器会将输出写入具有给定名称的单个`ZIP`格式存档文件。请注意，如果输出存档已存在，则会被覆盖; 编译器不够智能，无法将文件添加到现有存档中。
